...
def transfer_funds(conn, fromId, toId, amount):
    with conn.cursor() as cur:

        # Check the current balance.
        cur.execute("SELECT balance FROM accounts WHERE id = %s", (fromId,))
        from_balance = cur.fetchone()[0]
        if from_balance < amount:
            raise RuntimeError(
                f"Insufficient funds in {fromId}: have {from_balance}, need {amount}"
            )

        # Perform the transfer.
        cur.execute(
            "UPDATE accounts SET balance = balance - %s WHERE id = %s", (amount, fromId)
        )
        cur.execute(
            "UPDATE accounts SET balance = balance + %s WHERE id = %s", (amount, toId)
        )
...
def main():
    ...
    try:
        run_transaction(conn, lambda conn: transfer_funds(conn, fromId=1, toId=2, amount=100))
        run_transaction(conn, lambda conn: transfer_funds(conn, fromId=1, toId=3, amount=200))
